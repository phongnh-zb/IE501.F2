...
const Dashboard = (() => {
  ...
  let _drawn = { donut: false, scatter: false, module: false };
  ...
  function _scatter(rows) {
    ...
    const traces = [0, 1, 2, 3].map(tier => {
      ...
      return {
        ...
        text: pts.map(d => d.id),
        hovertemplate: '<b>%{text}</b><br>Score: %{x:.1f}<br>Clicks: %{y:,}<extra>' + TIER[tier].name + '</extra>',
  ...
  /* ── per-module risk bar chart ───────────────────────────────────────── */

  function _moduleChart(rows) {
    // Count critical + high per module
    const counts = {};
    rows.forEach(d => {
    ...
    // Sort modules by total urgent count descending
    const modules = Object.entries(counts)
      .map(([mod, c]) => ({ mod, crit: c.crit, high: c.high, total: c.crit + c.high }))
      .filter(x => x.total > 0)
    ...
    const layout = {
      ...
      xaxis: {
        title: { text: 'Students', standoff: 8 },
        gridcolor: '#edf2f7',
        zeroline: false,
        tickfont: { family: "'Space Mono', monospace", size: 10 },
      },
      ...
      legend: { orientation: 'h', y: -0.18, font: { size: 11 }, itemgap: 16 },
    };
  ...
  function _attentionList(rows) {
    const urgent = rows
      .filter(d => d.risk >= 2)
      .sort((a, b) => {
        // Withdrew-early students always top
        if (b.withdrew_early !== a.withdrew_early) return b.withdrew_early - a.withdrew_early;
        // Then by risk tier desc, then score asc
    ...
    el.innerHTML = urgent.map(d => {
      ...
      const subPct     = Math.max(0, Math.min(100, (d.submission_rate ?? 0) * 100));
      const scoreColor = d.risk === 3 ? '#dc2626' : '#ea580c';
      const subColor   = subPct < 40 ? '#dc2626' : subPct < 70 ? '#d97706' : '#059669';
      ...
      return `
        <a class="attention-row" href="/students?search=${encodeURIComponent(d.id)}">
          ...
          <span class="attention-badge-col">
            <span class="badge ${t.badge}">${t.name}</span>
          </span>
        </a>`;
    }).join('');
  }
